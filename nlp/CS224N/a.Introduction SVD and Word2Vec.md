...
这显然是一个很大的矩阵 $$\mathbb{R}^{|V| \times M}$$ ，它的规模是和文档数量 $$M$$ 成正比关系。因此我们可以尝试更好的方法。
...
- $$ \frac{\sum_{i=1}^{k} \sigma_{i}}{\sum_{i=1}^{|V|} \sigma_{i}} $$ 表示第一个 $$k$$ 维包含的方差量
...
我们创建两个矩阵， $$\mathcal{v} \in \mathbb{R}^{n \times|V|}$$ 和 $$\mathcal{u} \in \mathbb{R}^{|V| \times n}$$。其中：
...
分解为以下步骤：
- 我们为大小为 $$m$$ 的输入上下文词汇，生成one-hot词向量$$\left(x^{(c-m)}, \cdots, x^{(c-1)}, x^{(c+1)}, \cdots, x^{(c+m)}\in \mathbb{R}^{|V|}\right)$$
...
- 计算分数向量 $$z=\mathcal{U} \hat{v} \in \mathbb{R}^{|V|}$$ 。相似的词对向量的点积值大，这会令相似的词更为靠近，从而获得更高的分数
- 将分数通过softmax转换为概率 $$\hat{y}=\operatorname{softmax}(z) \in \mathbb{R}^{|V|}$$
-  我们希望生成的概率 $$\hat{y} \in \mathbb{R}^{|V|}$$ 与实际的概率 $$y \in \mathbb{R}^{|V|}$$ （实际是one-hot表示）越接近越好（我们后续会构建交叉熵损失函数并对其进行迭代优化）
